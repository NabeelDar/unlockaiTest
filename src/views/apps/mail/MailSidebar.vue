...
<template>
  <div>
    <Button class="mb-5 w-full" label="Compose New" outlined @click="navigate"></Button>
    <div class="overflow-auto">
      <ul class="flex flex-row md:flex-column gap-1 md:gap-2 list-none m-0 p-0 overflow-auto">
        <li
            ...
            @click="navigate(item)"
        >
          <i :class="[item.icon, { 'text-primary-50': router.currentRoute.value.path === item.routerLink }]"
             class="md:mr-3 text-600 transition-duration-150 text-lg"></i>
          <span :class="{ 'text-primary-50': router.currentRoute.value.path === item.routerLink }"
          ...
          <span v-if="item.badge"
                ...
                style="border-radius: 2rem; width: 23px; height: 23px">{{ item.badge }}</span>
        </li>
      </ul>
    </div>
  </div>
