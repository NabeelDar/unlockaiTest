...
<template>
  <div class="card">
    ...
    <p>Colors are exported as CSS variables and used with the standard <i>var</i> syntax such as
      <i>var(--text-color)</i>.</p>

    <pre class="app-code"><code>&lt;span :style="{color:var(--text-color)}"&gt;&lt;/span&gt;</code></pre>

    <h4>PrimeFlex Integration</h4>
    <p>Color utility classes of PrimeFlex such as background, text and border use the exported CSS variables from the
      theme so PrimeFlex or PrimeBlocks are perfectly compatible with the provided themes.</p>
      ...
      <div class="flex flex-wrap">
        <div v-for="color in colors" :key="color" class="flex flex-column mr-6 mb-6">
          <div v-for="shade in shades" :key="shade">
            <div v-if="shade !== 0" :style="{ backgroundColor: 'var(--' + color + '-' + shade, color: shade > 500 ? '#fff' : '#000' }"
                 class="flex align-items-center w-19rem p-3 font-bold">
              {{ color }}-{{ shade }}
            </div>
          </div>
        </div>
    ...
    <h4>Surfaces</h4>
    <p>In addition, a theme brings a special palette called surfaces that can be used as the base when designing the
      surface layers and separators.</p>
    <div class="card">
      <div class="flex flex-column">
        <div v-for="shade in shades" :key="shade">
          <div :style="{ backgroundColor: 'var(--surface-' + shade, color: darkMode ? (shade > 400 ? '#000' : '#fff') : shade > 500 ? '#fff' : '#000' }"
               class="flex align-items-center w-19rem p-3 font-bold">
            surface-{{ shade }}
          </div>
        </div>
